...
def test_build_tick_rows_emits_a_flat_baseline_for_every_player() -> None:
    ...
    assert all(t.performance_rating == 6.5 for t in baseline)
...
def test_build_tick_rows_moves_the_scorer_and_marks_them_impacted() -> None:
    ...
    assert any(t.current_price > 50.0 for t in scorer_event_ticks)  # goal moved the price up from base
...
def test_build_tick_rows_grants_clean_game_bonus_to_clean_starters_only() -> None:
    ...
    expected_rating = round(6.5 + bonus / 4.0, 2)
    assert all(t.performance_rating == expected_rating for t in ticks if t.ts == _FT_TS)


def test_dedup_ticks_keeps_the_last_write_per_player_ts() -> None:
    ts = datetime(2026, 6, 11, 19, 0)
    rows = [
        TickRow(1, ts, None, 50.0, 6.5, "engine"),
        TickRow(1, ts, 100, 55.0, 7.0, "engine"),  # same (player, ts) → wins
        TickRow(2, ts, 100, 40.0, 6.0, "engine"),
    ]
...
def test_derive_daily_snapshots_opens_at_carried_close_and_skips_non_fixture_days() -> None:
    ...
    ticks = [
        TickRow(1, baseline_day, None, 50.0, 6.5, "engine"),  # day before, no fixture
        TickRow(1, d1_a, 100, 55.0, 7.0, "engine"),
        TickRow(1, d1_b, 100, 53.0, 7.0, "engine"),  # last tick of the day → close
    ]
