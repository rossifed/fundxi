...
export function compute_holding_metrics(holding: Holding, player: Player): HoldingMetrics {
  const market_value = player.value * holding.shares;
  const cost_basis = holding.average_buy_price * holding.shares;
  return {
    ...holding,
    current_price: player.value,
    market_value,
    cost_basis,
...
export function compute_portfolio_totals(
  holdings: Holding[],
  players_by_id: Map<number, Player>,
): PortfolioTotals {
  let total_value = 0;
  let total_cost = 0;
  for (const h of holdings) {
    const p = players_by_id.get(h.player_id);
    if (!p) continue;
    total_value += p.value * h.shares;
